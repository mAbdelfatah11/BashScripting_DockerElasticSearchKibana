...
encr_repo="$HOME/env/encryption-script"
qna_svc="qna"
     ...
     echo -e "-----------------------------------------"
       ...
       #ES Compose file
        if [[ ! -f "$1/docker-compose.yml" ]]; then echo "";
                read -p "Pass in the ElasticSearch Compose file location [Current: s3://wb-ai/qna/qna-deployment/docker-compose.yml] > " compose_file
       ...
       s3_resources=(
	       ...
	       do
		       ...
		       then
			       ...
			       read -p "Pass in file URI in S3 [ ex: s3://wb-ai/{object-location} ] > " loaded_file;
